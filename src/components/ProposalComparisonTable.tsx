...
export const ProposalComparisonTable = ({ 
  ...
  // Calculate stats
  const { lowestPrice, fastestTimeline } = useMemo(() => {
    if (proposals.length === 0) return { lowestPrice: null, fastestTimeline: null };
    return {
      lowestPrice: Math.min(...proposals.map(p => p.price)),
      fastestTimeline: Math.min(...proposals.map(p => p.timeline_days)),
    };
  }, [proposals]);
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50">
              ...
              <TableHead className="text-right">לו״ז (ימים)</TableHead>
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <span>{proposal.timeline_days}</span>
                        {isFastest && (
                          <Badge variant="accent" className="text-xs px-1 py-0">
                            מהיר
                          </Badge>
                        )}
                      </div>
            ...
            >
              <CardContent className="p-3">
                ...
                <div className="grid grid-cols-2 gap-2 mb-2 text-sm">
                  ...
                  <div>
                    <span className="text-muted-foreground text-xs">לו״ז:</span>
                    <div className="flex items-center gap-1">
                      <Clock className="w-3 h-3 text-muted-foreground" />
                      <span>{proposal.timeline_days} ימים</span>
                      {isFastest && <Badge variant="accent" className="text-xs px-1 py-0">מהיר</Badge>}
                    </div>
                  </div>
                </div>
