-- Update handle_new_user function to set is_active to false for new advisors
...
BEGIN
  ...
  -- If user is an advisor, create advisor record with is_active = false (pending approval)
...
-- Update generate_project_recommendations to ensure it only recommends active advisors
...
BEGIN
  ...
  -- IMPORTANT: Only include active (approved) suppliers
  FOR supplier_rec IN 
