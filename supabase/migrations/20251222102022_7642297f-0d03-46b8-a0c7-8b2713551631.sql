CREATE OR REPLACE FUNCTION public.submit_negotiation_response(p_session_id uuid, p_updated_line_items jsonb, p_consultant_message text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- 3. Try to get previous version, fall back to proposal data if not found
  ...
  -- 6. Create version-scoped line items with updated prices
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_updated_line_items)
  LOOP
