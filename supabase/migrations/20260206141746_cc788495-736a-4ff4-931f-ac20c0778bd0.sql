-- Fix: Use advisor's user_id instead of advisor_id for created_by column
-- The created_by column references auth.users(id), not advisors(id)
...
DECLARE
  ...
  v_advisor_user_id uuid;  -- Store advisor's auth user ID
...
BEGIN
  ...
  -- Get the advisor's user_id for the created_by field (FIX: was using advisor_id before)
  ...
  -- Create new proposal version (FIX: use v_advisor_user_id instead of consultant_advisor_id)
