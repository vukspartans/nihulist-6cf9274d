...
serve(withCronSecurity('payment-deadline-reminder', async (_req) => {
  ...
  // 1. Day-after-due-date follow-up
  ...
  if (overdueRequests && overdueRequests.length > 0) {
    ...
    const { data: projects } = await supabase.from('projects').select('id, name, owner_id').in('id', projectIds);
    ...
    for (const ownerId of ownerIds) {
      ...
      const rows = relevantRequests.map((r: any) => {
        ...
        const amount = new Intl.NumberFormat('he-IL', { style: 'currency', currency: 'ILS', maximumFractionDigits: 0 }).format(r.total_amount || r.amount);
        return `<tr>
  ...
  return new Response(
    JSON.stringify({ ok: true, overdue: overdueCount, approaching: approachingCount }),
