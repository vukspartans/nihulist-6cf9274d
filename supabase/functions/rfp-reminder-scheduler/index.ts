...
// Helper to get team member emails
...
serve(async (req) => {
  ...
  try {
    ...
    // Target: status = 'sent' (never opened)
    // =====================================================
    const threeDaysAgo = new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000)
    const threeDaysAgoEnd = new Date(threeDaysAgo.getTime() + 60 * 60 * 1000) // +1 hour window
    ...
    if (stage1Error) {
      ...
      for (const invite of stage1Invites || []) {
        try {
          ...
          if (daysRemaining <= 0) continue // Skip expired

          const loginUrl = 'https://www.billding.ai/auth?type=advisor&mode=login'
          ...
          if (emailError) {
            console.error(`[RFP-Reminder] Stage 1 email failed for ${invite.id}:`, emailError)
            ...
            await supabase.from('activity_log').insert({
              ...
              meta: { stage: 1, type: 'unopened_3_day' },
            })

            console.log(`[RFP-Reminder] Stage 1 sent to ${invite.email} for project ${project.name}`)
            results.push({ stage: 1, inviteId: invite.id, advisorEmail: invite.email, success: true })
          }
        } catch (err) {
          console.error(`[RFP-Reminder] Stage 1 error for invite ${invite.id}:`, err)
    ...
    // Target: status = 'sent' or 'opened' (no submission yet)
    // =====================================================
    ...
    if (stage2Error) {
      ...
      for (const invite of stage2Invites || []) {
        try {
          ...
          const daysRemaining = daysBetween(now, new Date(invite.deadline_at))
          if (daysRemaining <= 2) continue // Will get final reminder instead
          ...
          if (emailError) {
            console.error(`[RFP-Reminder] Stage 2 email failed for ${invite.id}:`, emailError)
            ...
            await supabase.from('activity_log').insert({
              ...
              entity_id: invite.id,
              project_id: project.id,
              meta: { stage: 2, type: 'no_submission_7_day' },
            })

            console.log(`[RFP-Reminder] Stage 2 sent to ${invite.email} for project ${project.name}`)
            results.push({ stage: 2, inviteId: invite.id, advisorEmail: invite.email, success: true })
          }
        } catch (err) {
          console.error(`[RFP-Reminder] Stage 2 error for invite ${invite.id}:`, err)
    ...
    // Target: status = 'sent', 'opened', or 'in_progress'
    // =====================================================
    ...
    if (stage3Error) {
      ...
      for (const invite of stage3Invites || []) {
        try {
          ...
          if (emailError) {
            console.error(`[RFP-Reminder] Stage 3 email failed for ${invite.id}:`, emailError)
            ...
            await supabase.from('activity_log').insert({
              ...
              entity_id: invite.id,
              project_id: project.id,
              meta: { stage: 3, type: 'final_deadline_24_48h', hours_remaining: hoursRemaining },
            })

            console.log(`[RFP-Reminder] Stage 3 sent to ${invite.email} for project ${project.name}`)
            results.push({ stage: 3, inviteId: invite.id, advisorEmail: invite.email, success: true })
          }
        } catch (err) {
          console.error(`[RFP-Reminder] Stage 3 error for invite ${invite.id}:`, err)
    ...
    console.error('[RFP-Reminder] Fatal error:', error)
