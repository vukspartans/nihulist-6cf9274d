/**
 * COMPLETE INDEX.TS FOR SUPABASE DASHBOARD - WITH AUTOMATIC FALLBACK
 * 
 ...
 * 
 * KEY FEATURES:
 * - Supports both OpenAI (GPT-4o) and Google AI Studio (Gemini 1.5 Flash 002)
 * - Automatic fallback: If Google AI fails (404 or other errors), automatically switches to OpenAI
 * - Batch evaluation: Analyzes and ranks multiple proposals for comparison
 * - Single evaluation: Evaluates individual proposals against project requirements
 * - Uses latest Gemini model: gemini-1.5-flash-002 (not deprecated gemini-1.5-flash)
 */
...
// ============================================================================
// GOOGLE AI STUDIO HELPER (inline - uses gemini-1.5-flash-002)
// ============================================================================
...
async function callGoogleAIStudio(
  ...
  // FIXED: Use gemini-1.5-flash-002 (latest flash model, supported on v1 API)
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[Evaluate] Initial AI provider: ${aiProvider}`);

    // Get API keys for both providers (for fallback)
    ...
    try {
      ...
      console.log('[Evaluate] Google API key not available');
    }
    
    try {
      openAiApiKey = getOpenAIAPIKey();
    } catch (e) {
      console.log('[Evaluate] OpenAI API key not available');
    }

    // If initial provider's key is missing, switch to the other
    if (aiProvider === 'google' && !googleApiKey) {
      if (openAiApiKey) {
        console.log('[Evaluate] Google API key missing, switching to OpenAI');
        aiProvider = 'openai';
      } else {
        return new Response(
          JSON.stringify({
            success: false,
            error: 'No AI API keys available. Please configure OPENAI_API_KEY or GOOGLE_API_KEY',
      ...
      if (googleApiKey) {
        console.log('[Evaluate] OpenAI API key missing, switching to Google');
        aiProvider = 'google';
      } else {
        return new Response(
          JSON.stringify({
    ...
    // ATTEMPT 1: Try the selected AI provider first
    let evaluationResult: EvaluationResult;
    let finalProvider: 'openai' | 'google';
    ...
    try {
      if (aiProvider === 'openai') {
        ...
        evaluationResult = await Promise.race([
          callOpenAI(SYSTEM_INSTRUCTION, userContent, openAiApiKey),
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('Evaluation timeout')), EVALUATION_TIMEOUT_MS)
          ),
        ]);
        finalProvider = 'openai';
        finalModel = Deno.env.get('OPENAI_MODEL') || 'gpt-4o';
        console.log('[Evaluate] Successfully used OpenAI');
      } else {
        if (!googleApiKey) throw new Error('Google API key not available');
        evaluationResult = await Promise.race([
          callGoogleAIStudio(SYSTEM_INSTRUCTION, userContent, googleApiKey),
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('Evaluation timeout')), EVALUATION_TIMEOUT_MS)
          ),
        ]);
        finalProvider = 'google';
        finalModel = Deno.env.get('GEMINI_MODEL') || 'gemini-1.5-flash-002';
        console.log('[Evaluate] Successfully used Google AI Studio');
      }
    } catch (firstAttemptError) {
      // FALLBACK: If Google AI fails (404 or other error), try OpenAI if available
      const errorMessage = firstAttemptError instanceof Error ? firstAttemptError.message : String(firstAttemptError);
      const is404Error = errorMessage.includes('404') || errorMessage.includes('not found');
      
      console.log(`[Evaluate] First attempt failed (${aiProvider}):`, errorMessage.substring(0, 200));
      
      if (aiProvider === 'google' && openAiApiKey) {
        console.log('[Evaluate] Attempting fallback to OpenAI...');
        try {
          evaluationResult = await Promise.race([
            callOpenAI(SYSTEM_INSTRUCTION, userContent, openAiApiKey),
            new Promise<never>((_, reject) =>
          ...
          finalProvider = 'openai';
          finalModel = Deno.env.get('OPENAI_MODEL') || 'gpt-4o';
          console.log('[Evaluate] Fallback to OpenAI successful');
        } catch (fallbackError) {
          console.error('[Evaluate] Fallback to OpenAI also failed:', fallbackError);
          throw firstAttemptError; // Throw original error
        }
      } else if (aiProvider === 'openai' && googleApiKey) {
        console.log('[Evaluate] Attempting fallback to Google AI...');
        try {
          evaluationResult = await Promise.race([
            callGoogleAIStudio(SYSTEM_INSTRUCTION, userContent, googleApiKey),
            new Promise<never>((_, reject) =>
          ...
          finalProvider = 'google';
          finalModel = Deno.env.get('GEMINI_MODEL') || 'gemini-1.5-flash-002';
          console.log('[Evaluate] Fallback to Google AI successful');
        } catch (fallbackError) {
          console.error('[Evaluate] Fallback to Google AI also failed:', fallbackError);
          throw firstAttemptError; // Throw original error
        }
      } else {
      ...
      await supabase
        ...
        .update({
          ...
          evaluation_metadata: {
            ...
            provider: finalProvider === 'openai' ? 'openai' : 'google-ai-studio',
            temperature: 0.0,
            evaluation_time_ms: evaluationTime,
            fallback_used: finalProvider !== aiProvider,
          },
        })
