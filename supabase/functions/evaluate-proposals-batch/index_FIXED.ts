...
// Google AI Studio helper function (inline - no need for separate file)
...
// OpenAI helper function (for faster batch processing)
...
serve(async (req) => {
  ...
  try {
    ...
    // Determine AI provider (OpenAI is preferred if available, faster for batch)
    ...
    let apiKey: string;
    try {
      apiKey = aiProvider === 'openai' ? getOpenAIAPIKey() : getGoogleAPIKey();
    } catch (error) {
      return new Response(
        JSON.stringify({
          success: false,
