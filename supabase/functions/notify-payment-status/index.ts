...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'milestone_unlocked' && milestone_id) {
      // Notify consultant that a milestone is now due (can submit invoice)
      ...
      if (!milestone?.project_advisor?.advisors?.user_id) {
        return new Response(JSON.stringify({ error: 'Advisor not found' }), { status: 404, headers: corsHeaders });
      }
    ...
    if (type === 'payment_marked_paid' && payment_request_id) {
      // Notify consultant that payment was marked as paid -> issue tax invoice
      ...
      // Get investor/entrepreneur name
    ...
    if (type === 'status_changed' && payment_request_id) {
      // Generic status change notification to consultant
      const { data: request } = await supabase
        .from('payment_requests')
        .select(`
      ...
      if (!request?.project_advisor?.advisors?.user_id) {
        return new Response(JSON.stringify({ ok: true, skipped: 'no advisor' }), { headers: corsHeaders });
      }
